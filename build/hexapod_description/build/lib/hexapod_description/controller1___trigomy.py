...
class Controller(Node):
    def __init__(self):
        ...
        self.yHome = 0.0
        self.zHome = -0.09221  # -0.0883 #-0.221      #-0.0922   #-0.0839
        ...
        try:
            while rclpy.ok():
               ...
               xyz_in_world=self.FromWorldToRobotBase(legnumber,self.array_of_position[legnumber],self.array_of_position[legnumber +6], self.array_of_position[legnumber+12] +lift_z)
               xyz_in_robot_base=self.FromRobotBaseToWorld(legnumber,xyz_in_world[0],xyz_in_world[1],xyz_in_world[2])
               qact=self.inverseKinematicsTrigo(xyz_in_robot_base[0],xyz_in_robot_base[1],xyz_in_robot_base[2])
               self.commands_Position_leg024.data[legnumber]=qact[0]
               self.commands_Position_leg024.data[legnumber+6]=qact[1]
               self.commands_Position_leg024.data[legnumber+12]=qact[2]
    ...
    def my_callback(self, msg: JointState):
        ...
        if self.counter_for_init_jointsValue == 0:
            ...
            xack=self.forwardKinematics(msg.position[legnumber],msg.position[legnumber+6],msg.position[legnumber+12])
