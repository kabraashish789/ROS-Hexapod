...
class Controller(Node): 
    def __init__(self):
        rclpy.init()
        super().__init__('pub_motor_data2')
        ...
        try:
         while rclpy.ok():
            ...
            N=18 # Number of iterations to achieve the desired goal, play with to change the speed
            ...
            PStart_leg1=self.forwardKinematics(self.leg1_q1_start, self.leg1_q2_start, self.leg1_q3_start)
            PACt_leg1=PStart_leg1

            PStart_leg3=self.forwardKinematics(self.leg3_q1_start, self.leg3_q2_start, self.leg3_q3_start)
            PACt_leg3=PStart_leg3

            PStart_leg5=self.forwardKinematics(self.leg5_q1_start, self.leg5_q2_start, self.leg5_q3_start)
            ...
            for i in range(N):
              ...
              time.sleep(0.001)
            # next -movement
            XGoal_leg1=np.array([ self.xHOme ,self.yHome + For_Y , self.zHome +lift_z])
            deltaX_leg1=np.array([0.0, 0.0, 0.0])
            path_leg1=np.array([0.0, 0.0, 0.0])

            XGoal_leg3=np.array([self.xHOme + Forw_X,self.yHome , self.zHome +lift_z])
            deltaX_leg3=np.array([0.0, 0.0, 0.0])
            path_leg3=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg5=np.array([self.xHOme,self.yHome - For_Y,self.zHome + lift_z])
            deltaX_leg5=np.array([0.0, 0.0, 0.0])
            path_leg5=np.array([0.0, 0.0, 0.0])
            ...
            XGoal_leg1=np.array([ self.xHOme, self.yHome+For_Y, self.zHome])
            deltaX_leg1=np.array([0.0, 0.0, 0.0])
            path_leg1=np.array([0.0, 0.0, 0.0])

            XGoal_leg3=np.array([self.xHOme + Forw_X, self.yHome, self.zHome])
            deltaX_leg3=np.array([0.0, 0.0, 0.0])
            path_leg3=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg5=np.array([self.xHOme, self.yHome - For_Y, self.zHome])
            deltaX_leg5=np.array([0.0, 0.0, 0.0])
            path_leg5=np.array([0.0, 0.0, 0.0])
            ...
            for i in range(N):
              ...
              dq_leg0=self.inverseKinematics(qAct_leg0[0],qAct_leg0[1],qAct_leg0[2], deltaX_leg0[0], deltaX_leg0[1], deltaX_leg1[2])
              ...
              time.sleep(0.001)      
            # next -movement
            XGoal_leg0=np.array([ self.xHOme - Forw_X, self.yHome, lift_z + self.zHome])
            deltaX_leg0=np.array([0.0, 0.0, 0.0])
            path_leg0=np.array([0.0, 0.0, 0.0])

            XGoal_leg2=np.array([self.xHOme, self.yHome + For_Y, lift_z + self.zHome])
            deltaX_leg2=np.array([0.0, 0.0, 0.0])
            path_leg2=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg4=np.array([self.xHOme,self.yHome - For_Y, lift_z + self.zHome])
            deltaX_leg4=np.array([0.0, 0.0, 0.0])
            path_leg4=np.array([0.0, 0.0, 0.0])
            ...
            XGoal_leg0=np.array([self.xHOme - Forw_X, self.yHome, self.zHome])
            deltaX_leg0=np.array([0.0, 0.0, 0.0])
            path_leg0=np.array([0.0, 0.0, 0.0])

            XGoal_leg2=np.array([self.xHOme,self.yHome + For_Y, self.zHome])
            deltaX_leg2=np.array([0.0, 0.0, 0.0])
            path_leg2=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg4=np.array([self.xHOme,self.yHome - For_Y, self.zHome])
            deltaX_leg4=np.array([0.0, 0.0, 0.0])
            path_leg4=np.array([0.0, 0.0, 0.0])
