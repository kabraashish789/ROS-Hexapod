...
class Controller(Node): 
    def __init__(self):
        ...
        coxa=l1
        d1=0.0643
        self.xHOme= 0.1083     #0.1095  #0.1297
        self.yHome= 0.0 
        ...
        self.position_pub1 = self.create_publisher( Float64, "/my_position_controller1/commands",1)
        self.position_pub2 = self.create_publisher( Float64, "/my_position_controller2/commands",1)
        self.position_pub3 = self.create_publisher( Float64, "/my_position_controller3/commands",1)
        ...
        self.commands_Position_leg01=Float64()
  
        self.commands_Position_leg02=Float64()
      
        self.commands_Position_leg03=Float64()
        ...
        try:
         while rclpy.ok():
            ...
            if (For_Y>=2.0):
               self.get_logger().info(f"shuting down after 5%f. iterations {(iteration)}")
               rclpy.shutdown()
            ...
            legnumber=0
            xyz_from_base_to_world=self.FromRobotBaseToWorld(legnumber,self.leg0_xHome,self.leg0_yHome,self.leg0_zHome)
            xyz_from_world_to_base=self.FromWorldToRobotBase(legnumber,xyz_from_base_to_world[0],xyz_from_base_to_world[1],xyz_from_base_to_world[2] +lift_z  )
            ...
            for i in range(N):
              ...
              self.commands_Position_leg01.data=qAct_leg0[0]
              self.commands_Position_leg02.data=qAct_leg0[1] 
              self.commands_Position_leg03.data=qAct_leg0[2]
     
              time.sleep(0.01)
              self.position_pub1.publish( self.commands_Position_leg01) 
              self.position_pub2.publish( self.commands_Position_leg02) 
              self.position_pub3.publish( self.commands_Position_leg03) 
              self.get_logger().info(f"publishing leg01 lift {(self.commands_Position_leg01)}")
              PACt_leg0=self.forwardKinematics(qAct_leg0[0],qAct_leg0[1], qAct_leg0[2])
 
              
            # next -movement
            xyz_from_base_to_world=self.FromRobotBaseToWorld(legnumber,PACt_leg0[0],PACt_leg0[1],PACt_leg0[2])
            xyz_from_world_to_base=self.FromWorldToRobotBase(legnumber,xyz_from_base_to_world[0],xyz_from_base_to_world[1] + For_Y,xyz_from_base_to_world[2])
            ...
            for i in range(N):
              ...
              self.position_pub2.publish( self.commands_Position_leg02) 
              self.position_pub3.publish( self.commands_Position_leg03) 
              self.get_logger().info(f"publishing leg024 next move {(self.commands_Position_leg01)}")
              PACt_leg0=self.forwardKinematics(qAct_leg0[0],qAct_leg0[1], qAct_leg0[2])
            ...
            for i in range(N):
              ...
              self.position_pub2.publish( self.commands_Position_leg02) 
              self.position_pub3.publish( self.commands_Position_leg03) 
              self.get_logger().info(f"publishing leg024 back home {(self.commands_Position_leg01)}")
              PACt_leg0=self.forwardKinematics(qAct_leg0[0],qAct_leg0[1], qAct_leg0[2])
            ...
            self.get_logger().info(f" current joint values   \n {( self.array_of_q)} ")
    ...
    def my_callback(self, msg: JointState):
          ...
          if self.counter_for_init_jointsValue==0:
           ...
           for i in range(18):
            self.joint_state.position[i]=  msg.position[i]
           ...
           self.array_of_q[0]=self.leg0_q1_start
           self.array_of_q[6]=self.leg0_q2_start
           self.array_of_q[12]=self.leg0_q3_start'''
    ...
    def FromWorldToRobotBase(self, leg_number, xw,yw, zw):
       ...
       if leg_number==0 :
          ...
          xyz_in_robot_base[0]=-cos(orientation)*xw - sin(orientation)*yw -length*cos(orientation)*cos(orientation)-length*sin(orientation)*sin(orientation)
          xyz_in_robot_base[1]=sin(orientation)*xw - cos(orientation)*yw
          xyz_in_robot_base[2]=zw
       elif leg_number==2 :
    ...
    def FromRobotBaseToWorld(self, leg_number, xr,yr, zr):
       ...
       if leg_number==0 :
          ...
          xyz_in_world[0]=-cos(orientation)*xr + sin(orientation)*yr -length*cos(orientation)
          xyz_in_world[1]=-sin(orientation)*xr - cos(orientation)*yr -length*sin(orientation)
          xyz_in_world[2]=zr
       elif leg_number==2 :
