...
from math import sin, cos ,atan2 ,atan ,acos, sqrt,radians,pi 
import math
...
class Controller(Node): 
    def __init__(self):
        ...
        global l1,l2, l3,tibia, coxa, femur, alpha
        self.q1_start=radians(90)
        self.q2_start=radians(-45)
        self.q3_start=radians(130)
        alpha=radians(90)
        '''l1=0.03462
        l2=0.081542
        l3=146.604/1000'''
        ...
        self.yHome= 0.0  #0.1297
        self.xHOme= 0.1083
        self.zHome= 0.0922   #-0.0839
        ...
        self.position_pub = self.create_publisher( Float64MultiArray, "/my_position_controller/commands",18)
        ...
        try:
         while rclpy.ok():
            ...
            PStart_leg0=self.forwardKinematics(self.leg0_q1_start, self.leg0_q2_start, self.leg0_q3_start)
            PACt_leg0=PStart_leg0

            PStart_leg2=self.forwardKinematics(self.leg2_q1_start, self.leg2_q2_start, self.leg2_q3_start)
            PACt_leg2=PStart_leg2

            PStart_leg4=self.forwardKinematics(self.leg4_q1_start, self.leg4_q2_start, self.leg4_q3_start)
            PACt_leg4=PStart_leg4
            
              

            qAct_leg0=self.inverseKinematics(XGoal_leg0[0],XGoal_leg0[1],XGoal_leg0[2])
            
             
            qAct_leg2=self.inverseKinematics(XGoal_leg2[0],XGoal_leg2[1],XGoal_leg2[2])
            

            qAct_leg4=self.inverseKinematics(XGoal_leg4[0],XGoal_leg4[1],XGoal_leg4[2])
            ...
            time.sleep(0.001)      
            # next -movement
            XGoal_leg0=np.array([ self.xHOme , self.yHome + For_Y, lift_z + self.zHome])
            deltaX_leg0=np.array([0.0, 0.0, 0.0])
            path_leg0=np.array([0.0, 0.0, 0.0])

            XGoal_leg2=np.array([self.xHOme - Forw_X, self.yHome , lift_z + self.zHome])
            deltaX_leg2=np.array([0.0, 0.0, 0.0])
            path_leg2=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg4=np.array([self.xHOme + Forw_X,self.yHome , lift_z + self.zHome])
            deltaX_leg4=np.array([0.0, 0.0, 0.0])
            path_leg4=np.array([0.0, 0.0, 0.0])
            ...
            XGoal_leg0=np.array([self.xHOme, self.yHome + For_Y, self.zHome])
            deltaX_leg0=np.array([0.0, 0.0, 0.0])
            path_leg0=np.array([0.0, 0.0, 0.0])

            XGoal_leg2=np.array([self.xHOme - Forw_X,self.yHome, self.zHome])
            deltaX_leg2=np.array([0.0, 0.0, 0.0])
            path_leg2=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg4=np.array([self.xHOme + Forw_X,self.yHome, self.zHome])
            deltaX_leg4=np.array([0.0, 0.0, 0.0])
            path_leg4=np.array([0.0, 0.0, 0.0])
            ...
            qAct_leg1=self.inverseKinematics(XGoal_leg1[0],XGoal_leg1[1],XGoal_leg1[2])
              

            qAct_leg3=self.inverseKinematics(XGoal_leg3[0],XGoal_leg3[1],XGoal_leg3[2])
              

            qAct_leg5=self.inverseKinematics(XGoal_leg5[0],XGoal_leg5[1],XGoal_leg5[2])
            ...
            time.sleep(0.001)
            # next -movement
            XGoal_leg1=np.array([ self.xHOme - Forw_X,self.yHome, self.zHome +lift_z])
            deltaX_leg1=np.array([0.0, 0.0, 0.0])
            path_leg1=np.array([0.0, 0.0, 0.0])

            XGoal_leg3=np.array([self.xHOme, self.yHome -For_Y,self.zHome + lift_z])
            deltaX_leg3=np.array([0.0, 0.0, 0.0])
            path_leg3=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg5=np.array([self.xHOme + Forw_X,self.yHome ,self.zHome + lift_z])
            deltaX_leg5=np.array([0.0, 0.0, 0.0])
            path_leg5=np.array([0.0, 0.0, 0.0])
            ...
            XGoal_leg1=np.array([ self.xHOme - Forw_X,self.yHome, self.zHome ])
            deltaX_leg1=np.array([0.0, 0.0, 0.0])
            path_leg1=np.array([0.0, 0.0, 0.0])

            XGoal_leg3=np.array([self.xHOme, self.yHome -For_Y,self.zHome ])
            deltaX_leg3=np.array([0.0, 0.0, 0.0])
            path_leg3=np.array([0.0, 0.0, 0.0])
           

            XGoal_leg5=np.array([self.xHOme + Forw_X,self.yHome ,self.zHome ])
            deltaX_leg5=np.array([0.0, 0.0, 0.0])
            path_leg5=np.array([0.0, 0.0, 0.0])
            ...
            self.position_pub.publish( self.commands_Position_leg135) 
            self.get_logger().info(f"publishing leg135 back {(self.commands_Position_leg135)}")
    ...
    def forwardKinematics(self ,q1,q2,q3):
       ...
       '''xAck[0] = (coxa*cos(q1Rad+(pi/2)))+(femur*cos(q2Rad)*cos(q1Rad+(pi/2)))+(tibia*cos(q2Rad)*cos(q3Rad)*cos(q1Rad+(pi/2)))-(tibia*cos(q1Rad+(pi/2))*sin(q2Rad)*sin(q3Rad))
       xAck[1] = (coxa*sin(q1Rad+(pi/2)))+(femur*cos(q2Rad)*sin(q1Rad+(pi/2)))+(tibia*cos(q2Rad)*cos(q3Rad)*sin(q1Rad+(pi/2)))-(tibia*sin(q2Rad)*sin(q3Rad)*sin(q1Rad+(pi/2)))
       xAck[2] = (femur*sin(q2Rad))+(tibia*cos(q2Rad)*sin(q3Rad))+(tibia*cos(q3Rad)*sin(q2Rad))'''
