...
class Controller(Node): 
    def __init__(self):
        ...
        try:
         while rclpy.ok():
            ...
            self.commands_Velocity_leg024.data[legnumber]=0.0001
            self.commands_Velocity_leg024.data[legnumber +6]=0.0001
            self.commands_Velocity_leg024.data[legnumber+12]=0.0001
            xyz_from_base_to_world=self.FromWorldToRobotBase(legnumber,self.leg0_xHome,self.leg0_yHome,self.leg0_zHome)
            xyz_from_world_to_base=self.FromRobotBaseToWorld(legnumber,xyz_from_base_to_world[0],xyz_from_base_to_world[1],xyz_from_base_to_world[2] + lift_z  )
            ...
            for i in range(N):
              ...
              self.commands_Position_leg024.data[legnumber+ 6]=qAct_leg0[1]
              self.commands_Position_leg024.data[legnumber + 6 + 6]=qAct_leg0[2]
              dummy=0 
              
             # time.sleep(0.001)
              '''while myduration.to_msg().nanosec > (self._clock.now().nanoseconds - start_time) :
                 dummy+=1
                 self.get_logger().info(f" duration ? {(myduration.to_msg().nanosec)}") 
                 self.get_logger().info(f" (self._clock.now().nanoseconds - start_time) ? {(self._clock.now().nanoseconds - start_time)}") 
                 self.get_logger().info(f" self._clock.now().nanoseconds ? {(self._clock.now().nanoseconds)}") 
                 self.get_logger().info(f" dummy ? {(dummy)}")  
              self.get_logger().info(f"is alive ? {(self.position_pub.assert_liveliness())}")'''
            ...
            # next -movement
            xyz_from_base_to_world=self.FromWorldToRobotBase(legnumber,PACt_leg0[0],PACt_leg0[1],PACt_leg0[2])
            xyz_from_world_to_base=self.FromRobotBaseToWorld(legnumber,xyz_from_base_to_world[0],xyz_from_base_to_world[1] + For_Y,xyz_from_base_to_world[2])
            ...
            for i in range(N):
              ...
              '''while  myduration.to_msg().nanosec > (self.get_clock().now().nanoseconds - start_time().nanoseconds) :
                 dummy+=1
                 self.get_logger().info(f" dummy ? {(dummy)}")  
              self.get_logger().info(f"is alive ? {(self.position_pub.assert_liveliness())}")'''   
             # time.sleep(0.001)
    ...
    def my_callback(self, msg: JointState):
          ...
          if self.counter_for_init_jointsValue==0:
           ...
           xstart=self.forwardKinematics(msg.position[ legnumber],msg.position[legnumber +6],msg.position[legnumber +12])
           self.t=1
        
           self.leg0_xHome=xstart[0]
           self.leg0_yHome=xstart[1]
           self.leg0_zHome=xstart[2]
  
           self.leg0_q1_start=msg.position[legnumber]
           self.leg0_q2_start=msg.position[legnumber+6]
           self.leg0_q3_start=msg.position[legnumber+12]
