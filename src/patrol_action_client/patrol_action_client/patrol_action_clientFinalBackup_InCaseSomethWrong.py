...
def main(args=None):
    ...
    global qAct_first_leg, legnumber, GoToSwing, GoToStance, oneStepMotionIsFinished,oldX,oldY,oldZ
    GoToSwing=True
    GoToStance=False
    For_Y=30
    For_stance=100
    ...
    try:
         while rclpy.ok():
            ...
            while i<27:
                ...
                if position[i]==-1.2:
                    goStraight=True
                    action_client.get_logger().info(f" goStraight=  \n {( goStraight)} ")
                else:
                 z=position[i]
                 goStraight=False'''
                ...
                ys-=forrwardY
                action_client.get_logger().info(f" x=  \n {( x)} ")
                xyz_in_robot_base_first_leg = action_client.FromRobotBaseToWorld(
                    stance[0], xs, ys, zs )
                xyz_in_robot_base_second_leg = action_client.FromRobotBaseToWorld(
                    stance[1], xs, ys, zs )
            ...
            i=0    
            for i in range (6):
             action_client.array_of_position[i]=action_client.xHOme
             action_client.array_of_position[i+6]=action_client.yHome
             action_client.array_of_position[i+12]=action_client.zHome   
